...
**Disclaimer: We are still adjusting the codes for compatibility with the techniques that have changed since we performed the experiments.**
...
```
2. Clone the repositories of each converter for tabular data into Images and install their requirements.txt (clone the repos inside each converter folder)
```bash
